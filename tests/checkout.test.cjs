...
function setup(vercel = false, catalogOverride = catalog) {
  const calls = [];
...
function order(qty, shipping, coupon = '') {
  ...
  return {
    ...
    shipping: { amountCents: shipping }, coupon, discountCents,
...
test('rounding preserves exact cents across quantities and different products', async () => {
  ...
  const { post, calls } = setup(false, modified);
...
test('existing intermediate checkout requests remain compatible', async () => {
  const { post } = setup();
  assert.equal((await post({ items: order(1, 0).items, shipping: 'normal' })).status, 200);
});
